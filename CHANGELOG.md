...
- 结算回执成为 confirm 事务成功后的只读呈现投射：`_confirm()` 深拷贝已确认事实
  为候选，`command()` 只在持久化成功、未回滚时转正；保存失败/回滚/重复确认
  一律不产生回执。非胜利分支清空 `data.summary` 与 `settlement_id` 后，
  回执仍保留 `result_key`/`settlement_id`/`attempt_sequence` 与全部损益字段。
  不写存档、不改 schema、不新增第二套结算、UI 不重算伤亡与资源。
- 永久主城归来简报：视图转交 → 控制器暂存 → 主城重新可见时消费一次；
  三态标题（凯旋归城/部队归城/战损归城）、首屏五项指标、暂存警示带、
  折叠详情、`知道了` 与 `查看城市影响`。战时内城强制收起，
  刷新×10、面板往返、冷启动均不重复或重放。
- 经营回流优先级 前线暂存 > 伤员 > 实际粮食风险 > 城市概况，
  判据取自主城权威读模型；路由复用 R2B-0 五分组并聚焦现有控件，
  未知分组安全回退「概况」，只切 UI 不领取/不治疗/不建造/不调岗。
- 修复路由被刷新收回：`city_state_changed` 直连 `_refresh_read_model()`，
  `_sync_governance_groups()` 无条件按默认告警规则重排开合，
  导致「查看城市影响」打开的分组下一次刷新即收起。
  新增 `_governance_group_directive` 由同步点单点应用，
  玩家自己点分组开合或开合面板时交还布局控制权。
- 新增 11 段定向测试（115 项断言，覆盖指令 §7 十五场景与两分辨率）
  与 5 用例可见操作取证（真实窗口 + 合成鼠标事件，各独立进程与隔离存档链）；
  取证与验证脚本均带看门狗与硬超时，避免协程中断遗留无头实例。
- 复验收口（产品代码零改动）：在最终 HEAD 上重跑定向矩阵、取证矩阵与 §10 回归，
  逐项与既有记录相符；玩家存档哨兵跑前跑后一致（`deb4f150…`，3201 文件）。
  本轮原始日志/facts/重拍截图已落入交付包
  `TXWZS_R2B1_DELIVERY/RERUN_20260919_AT_7b3b3c1/`，使数字重新可与日志逐行比对。
  同时记录两处既有问题（不在本轮修改）：顶栏 `resource_summary`/`time_summary`
  多写者导致证据 08 不可逐字节复现（写者均早于基线 `7464898`）；
  存档门禁对已不存在的保护根仍判 PASS（空集哨兵恒等）。

# R2B-0 主操作层级与道路可靠性收口 — 2026-09-17

- 常规战役期间隐藏宏行军入口，战役主操作由 current_mainline_entry_button
  唯一承担；PREPARATION/ACTIVE/PENDING/COMPLETED 文案各归其位。
- 经营总览重组为五个折叠分组（概况默认展开，警报分组自动展开，
  其余一行摘要）；医疗/治理岗位拆行各归其组；hint 独立于面板显示并超时消失。
- 顶栏目标文案截断、PREPARATION 状态说明缩为一行。
- 修复 handle_escape 道路分支：有预览段时此前只清预览不退出放置模式；
  现三入口（按钮/Esc/右键）统一走 cancel_placing 完全退出。
- 进入放置/道路模式自动关闭建设目录，消除目录对取消/确认按钮的遮挡。
- 新增道路模式与取消的定向断言（verify_r2a1_governance_entry.gd 内）。
