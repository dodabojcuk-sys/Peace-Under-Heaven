...
已完成的定向覆盖包括：出发/抵达粮草守恒、重复点击、无路零写入、途中断路恢复、仓满待卸、Field 严格快照与旧快照兼容；独立 A/B/C 进程验证运输前、在途和完成后的保存恢复，以及完成态不重复入库。本轮补足仓满等待只在首次状态变化保存、真实维修在同一时间步完成时仅消费修复后的运输时间，并在大步、30 FPS、60 FPS 与不规则拆分间对照。入库成功后注入攻城同步失败、以及关键保存失败，均回滚资源、运输完成态和战区快照；恢复正常后只会入库一次。完成运输的地点详情现在持久显示“本批 20 粮已入库”，不会退回成“没有可运回粮草”或重复播放完成事件。图形 GUI 证据以真实地图按下/松开打开银渡详情，断言运输按钮可见、可用；Godot SceneTree 无法将合成系统鼠标分发到原生 `Button`，所以该按钮的连接动作信号覆盖被明确单列，不冒充 macOS 真实鼠标证据。没有关闭旧候选或创建新的长期候选。玩家验收保持 **OPEN**。详见 `docs/milestones/txwzs-field-tactics-r2/evidence/20260911-silverford-supply-r0/VERIFICATION.md`。
