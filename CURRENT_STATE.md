...
宏观军队抵达拒降敌城后，外城侧栏现在提供“进入战时围城”。该入口从原 `army_id`、原宏观 `order_id`、原编队快照、当前城门与守军构造 C0 请求；不会调用常态内城的出征流程，也不会再次扣粮、抽取黑石驻军或生成默认小队。`WarLoopState` schema 4 在同一围城记录内保存严格的 `wartime_handoff` 关系（事务 ID、阶段、终局 ID、活动会话快照），而军队、城市控制权、资源和围城事实仍由既有所有者持有。

接管关联处于 RESERVED、ACTIVE 或 RESULT_PENDING 时，统一世界时间跳过**这一场**围城的自动推进，其他外城世界事务仍按原循环执行。C0 每个非终局刻通过既有检查点入口写回该关联；重新进入可恢复同一会话。已实现撤退回写：按真实编队结果写回原军队，关闭原围城并建立既有合法返程，不新建城市军队。新的正式 smoke 已覆盖原编队/粮食不重复、关联持久化、时钟跳过、活动实例重开及撤退回写。胜利、全灭、结果待回写和**独立进程**恢复仍在同一阶段继续实现；当前不得把这项检查点称为完整战时攻防或完整冷恢复。
