...
定向验证覆盖合法到场/完工/新增视野、越界/水域/道路未连通/重复拒绝、赴工中工程师损失、旧快照不补塔及非法新字段严格拒绝；独立 A/B/C 进程覆盖在途、完工与完成态恢复且不会重复造塔。图形 GUI 证据包含工程师选择、松手后可见计划、到场施工和完工新增视野，保存在 `docs/milestones/field-tactics-r2/evidence/20260911-field-watchtower-r0/`。地图按下/松开是引擎 GUI 事件；侧栏开工按钮为可见、未禁用的已连接动作信号，不冒充 macOS 系统鼠标录像。现有候选窗口和隔离存档没有关闭或覆盖；玩家验收继续 **OPEN**。
