...
施工中的拒马、箭塔或瞭望台不再是不可破坏的占位物：敌军抵达同一路线时，会先中断该路线尚未完工的拒马、箭塔或瞭望台，记录可保存的 `INTERRUPTED` 状态，设施在此之前不提供减伤、火力或观察。每项战时施工/维修还会冻结一支真实已承诺小队作为施工分队；该分队撤退或失去行动能力时，工程同样进入中断态，不会凭空继续完工。维修必须重新绑定一支存活的既有参战小队，仍不创建第二套工程师或兵员账本。该状态的维修与其他受损设施共享既有正式事务和两刻工期；完成后才恢复为 `ACTIVE`。同类工事现在按“类型＋实际路线”唯一：玩家可在前线、侧翼各部署一座瞭望台或箭塔，面板只对当前所选路线显示已选状态；两条路线的观察与箭塔伤害都在同一战斗刻分别投影和结算，恢复后保留相同工事身份。守城还新增路线专属的刺钉陷阱：正式面板选择并确认后施工两刻，敌军实际抵达该路线时只触发一次 80 点真实伤害，消耗自身记录；围城来源被权威拒绝使用，恢复也不会重播历史触发。定向战斗刻验证覆盖“抵达 → 中断未完工拒马 → 无提前防护 → 施工分队退出 → 恢复快照 → 正式维修 → 完工恢复”、双路线同类工事和一次性陷阱；独立 H→I 进程链进一步从正式守城入口保存施工受阻记录，冷启动后确认仍无防护、通过可见维修入口完成恢复。这是路线内工事生命周期完善，不等于已完成道路断开或完整战时防守战役。
