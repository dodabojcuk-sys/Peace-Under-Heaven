...
施工中的拒马、箭塔或瞭望台不再是不可破坏的占位物：敌军抵达同一路线时，会先中断该路线尚未完工的拒马、箭塔或瞭望台，记录可保存的 `INTERRUPTED` 状态，设施在此之前不提供减伤、火力或观察。该状态的维修与其他受损设施共享既有正式事务和两刻工期；完成后才恢复为 `ACTIVE`。定向战斗刻验证覆盖“抵达 → 中断未完工拒马 → 无提前防护 → 恢复快照 → 正式维修 → 完工恢复”。这是路线内工事生命周期完善，不等于已完成工程人员实体、道路断开或完整战时防守战役。
