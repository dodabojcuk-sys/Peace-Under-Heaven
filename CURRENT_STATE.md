...
本阶段新增一个独立、有限的试玩配置：银渡城在新开局中有 `initial_stationed_reinforcements = 4` 名待编步兵。它既不是敌方守军、黑石驻军或训练队列，也不会随刷新、反复占领或读档重新生成。`FieldTacticsState` 保存地点剩余兵源；`ArmyRegistry` 继续独占编队人数、军队汇总、`army_id` 与军令历史；`ConstructionController` 在提交时重新核对银渡控制权、目标军队是否真实 `STATIONED`、容量和地点库存，并把扣地点兵源、按稳定 `formation_id` 顺序补编、V5 检查点发布放在同一回滚边界内。补员不创建军令、不转移编队、不扣粮。

银渡地点详情现在要求玩家先明确选择一支实际驻军，再显示当地剩余兵源、该军队每个编队的当前/上限人数、预览分配及“补充 X 人”按钮；在途、交战/非驻扎、敌方控制、满编、无兵源和错误地点均由同一权威预检给出原因。实际 GUI 地图点击用于打开银渡详情；Godot `SceneTree` 无法把合成系统鼠标交给原生侧栏 `Button`，因此驻军选择与补员动作在证据中准确标为“可见、未禁用的已连接按钮信号”，不是 macOS 物理鼠标证据。

定向验证已经覆盖：真实 Controller 出征、占领银渡城、补 4 人、重复补员零写入、从银渡续令；稳定多编队分配；错误军队/地点、保存故障全快照回滚；旧 Field 快照不补发兵源；以及独立 A/B/C Godot 进程的补员后、续令后和在途恢复。图形 GUI 证据与日志位于 `docs/milestones/txwzs-field-tactics-r2/evidence/20260911-stationed-reinforcement-r0/`。现有候选窗口与隔离存档未被关闭或覆盖；玩家验收继续 **OPEN**。
