...
- `P0_05_RUNTIME_BUILDING_LIFECYCLE_AND_SAFE_REMOVAL_ACCEPTED`
...
- `ConstructionController` 同时拥有运行时建筑记录、placement ID、放置顺序、占用格和安全移除生命周期的唯一权威状态。
- `BuildingSelectionController`：负责已放置测试建筑的统一命中、选择状态、琥珀色描边和详情面板字段，不拥有 `_input`。
...
- 静态占位控件忽略鼠标事件；建造按钮和可见详情面板接收交互，地图输入仍由根节点脚本统一协调。
...
- 只选择 `PlacedBuildings` 下已确认的中性测试建筑，不让每栋建筑各自处理输入。
...
- 当前 branch 为 `main`，实现基线为 `67b4609d15b6412f1d9dca171af31af390c14f0c`；验收状态以本文件所在提交为准。
...
- 未实现正式建筑数据、正式建造菜单、道路建造规则、建筑升级、生产、拆除、多城市、存档或正式美术。
