...
- `C0_REAL_COMBAT_GRAYBOX_READY_FOR_USER_EXPERIENCE_GATE`
...
- P1-C 已实现第 5／8／10／12 日压力、公开敌军与工事、城防减损、第 9 日恢复检查点和第 12 日推进限制。
- P1-D 已实现第一时代步兵、每日维护、次日训练、一个将领槽、三个将领原型、三个首图科技和一次紧急动员。
- P1-E 曾在真实战斗／结果契约门禁停止；该契约缺口已由独立 C0 灰盒解阻，但正式军令台串联仍未恢复。
...
- 用户已接受 `FIRST_MAP_VERTICAL_SLICE_V0`；当前正式门禁是提供或单独批准真实的最小战斗／结果契约后再继续 P1-E。
- 已完成本地原子提交：
  - `6fec5ca`：P1-A 道路连通与伐木经济。
  - `c953b9a`：P1-B 粮食、容量与每日结算。
  - `45acb9f`：P1-C 威胁时间线与第 12 日门禁。
...
- `UI/Shell/CityBar`：以最低限度原生控件显示步兵、征募、一个将领槽、三个科技选项、威胁和第 12 日恢复操作；不是正式 UI 设计。
...
- 伐木场成本 40 木材、联网后次日起产 18 木材；农田成本 45 木材、联网后次日起产 22 粮食；仓库成本 60 木材并为两种资源各增加 120 容量；瞭望塔成本 50 木材并增加 10 城防。
- 每日结算顺序为：校验日期 → 扣军队维护 → 完成上一日训练 → 生产 → 研究 +1 → 更新敌军／工事 → 压力事件 → 清理一日停产。
- 第 5、8、10 日执行公开压力事件；敌军按 32、40、48、56、64 增长，第 12 日禁止继续结束本日。
...
- 第 12 日可使用一次紧急动员：30 粮换 5 步兵；该操作不绕过日期门禁。
- V0 实施未进行数值微调；上述数值仍为用户已接受的首轮校准起点。

## P1-E 原战斗契约阻断与 C0 解阻

- P1-E 停止时仓库只有 `res://scenes/blank_map.tscn`，没有战斗或结果场景。
- C0 已新增独立开发灰盒 `res://scenes/c0_battle_graybox.tscn`，但没有把它伪装成正式军令台入口。
- `CommandPlatform` 的现有权威说明明确为“真实战役契约接入前不提供假入口”。
- `P1_FIRST_MAP_VERTICAL_SLICE_V0` 中的确定性战备公式只被授权用于自动平衡代理，文档同时明确它不是永久战斗公式。
- C0 使用独立真实 tick 演算解除了胜负生成、兵力事务、结果幂等和返回城市契约缺口；P1-E 仍需在用户体验门禁通过后再接正式军令台。
- 告示板仍不存在，未新增、重命名或伪装告示板，也未实现历战入口。

## C0 最小真实战斗灰盒

- P1-E 和 P1-F 继续暂停；C0 独立灰盒已完成自动验证，当前等待用户体验门禁。
...
- 独立灰盒标有 `C0 Battle Graybox`，用于体验两条路线和命令，不是正式 UI 或 P1-E 军令台串联。
...
- 未实现惯性、回弹、真实小地图、正式美术、正式军令台战斗串联或存档；C0 仅提供独立真实战斗灰盒。
- 未实现或宣称触控板 `InputEventPanGesture` 支持。
- 未实现建筑移动、升级、批量操作、撤回、多城市、后续时代兵种、装备、正式科技树 UI、告示板或历战入口。
- P1-E、P1-F、首图平衡模拟、AI 完整玩家路径和用户实体体验门禁均未完成，不得把 P1-A 至 P1-D 的自动验证描述为可玩首图已经通过。
...
- P1-C 自动验证：第 5／8／10／12 日边界、敌军与工事公开、城防减损、确定性停产、第 9 日检查点和第 12 日推进拒绝。
- P1-D 自动验证：基础／轮训征募、次日训练、军粮维护与供给恢复、一个将领槽、三个将领修正、科技前置、石器增产、检查点字段和一次紧急动员。
- P1-A 至 P1-D 回归共 8 个 Godot smoke runner 全部通过；Godot 4.5.1 headless 主场景启动和 editor scan 无脚本、场景或资源错误。
