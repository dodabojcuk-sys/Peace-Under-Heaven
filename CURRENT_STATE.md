...
宏观军队抵达拒降敌城后，外城侧栏现在提供“进入战时围城”。该入口从原 `army_id`、原宏观 `order_id`、原编队快照、当前城门与守军构造 C0 请求；不会调用常态内城的出征流程，也不会再次扣粮、抽取黑石驻军或生成默认小队。`WarLoopState` schema 5 在同一围城记录内保存严格的 `wartime_handoff` 关系（事务 ID、阶段、终局 ID、活动会话快照及待回写终局权威快照），而军队、城市控制权、资源和围城事实仍由既有所有者持有。

接管关联处于 RESERVED、ACTIVE 或 RESULT_PENDING 时，统一世界时间跳过**这一场**围城的自动推进，其他外城世界事务仍按原循环执行。C0 每个非终局刻通过既有检查点入口写回该关联；终局先保存同一份权威战果，再允许唯一的宏观回写。重新进入可恢复活动会话或待确认战果，不会重演战斗、重扣兵力或复制结果。已实现撤退回写：按真实编队结果写回原军队，关闭原围城并建立既有合法返程，不新建城市军队；真实 C0 战斗刻的胜利回写会一次更新目标控制权，并让同一支原军队驻扎，重复确认只返回同一既有结果。正式 smoke 现覆盖原编队/粮食不重复、关联持久化、时钟跳过、活动实例重开、待回写结果重开、撤退及胜利回写；独立 A→B→C→D 进程链覆盖活动接管保存、战斗恢复、待回写战果恢复、一次回写及回写后再次启动。全灭结果的专门路线、战时设施施工/受损/维修及完整防守仍在同一阶段继续实现；当前不得把这项检查点称为完整战时攻防。
