...
V5 当前为 schema 9：活动战时实例保存战斗刻、小队血量/位置、路线、已接受和待执行命令、撤退与任务事实，不保存 Node、动画、相机或 UI 选择。非终局战斗刻经既有运行时发布入口形成检查点；新场景或 V5 恢复后会从同一刻继续。V7/V8 旧档迁移为空工事/空会话快照，不赠送资源；没有旧会话事实的历史 ACTIVE 出征维持原有从 tick 0 重建行为。篡改位置、路线或身份的活动会话被严格拒绝，不能污染城市状态。
