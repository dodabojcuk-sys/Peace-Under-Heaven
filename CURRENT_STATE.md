...
守城的 `RESERVED` 编队现在也可在正式 C0 的可见路线按钮中调整前线／侧翼部署：Controller 在同一冻结事务内同步更新编队与 committed-force 路线，持久化成功才刷新；没有新出征、编队改名或守城粮草扣除。启动后立即重新锁定。新增守城 smoke 还用三支真实黑石编队经过可见的选队与“前进”按钮清除两路敌军，并确认唯一守城胜利结果与完好城门；它不等于完整防守战役或玩家体验验收。
