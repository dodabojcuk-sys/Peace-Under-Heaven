...
- `P1_CITY_TIME_AND_VIEWPORT_CORRECTION_READY_FOR_USER_GATE`
...
- 当前每日日长为 `60` 模拟秒，是集中、可测试的 V0 节奏参数，等待用户体验校准，不代表最终平衡。
...
- 本轮没有改变 C0 战斗规则、数值或用户体验门禁；P1-E、P1-F 仍暂停。
...
- 完整首图循环为：建设与接路 → 木材／粮食 → 募兵／科技／防御 → 自动时间推进／明确暂停 → 敌军骚扰和增强 → 提前或延后进攻 → 极限日停止城市时间。
...
- `UI/Shell/TopStatusBar/PauseButton`：唯一玩家时间控制；运行态显示“暂停”，暂停态显示“继续”，不提供快进或手动跨日。
...
- 第 5、8、10 日执行公开压力事件；敌军按 32、40、48、56、64 增长，第 12 日自动停止城市时间并进入最终战备。
...
- 本轮完整回归共 14 个 Godot smoke runner、814 条断言全部通过；新增覆盖自动时间、暂停语义、帧增量确定性、日界线幂等、威胁日期、侧栏输入、安全矩形和节点唯一性。
