...
地图点击敌城会打开只读地点详情；已选中驻军也可从侧栏打开其所在地详情。侧栏显示名称、外观类型、控制方、用途、建设能力，以及**仅已抵达并驻扎在该点**的真实军队和人数；在途军队不会计入。点击驻军只定位/切换查看对象，不下令、不扣粮；后续长按地点、选择条和松手续令仍沿用既有正式操作。敌方控制的城池继续不能作为我方快捷派遣起点，占领后旗帜/控制投影和操作资格同步变化。

定向 Macro March 回归现覆盖：敌城无副作用查看、占领后地点详情、驻军查看、从已占领城池经正式长按选择条续令、在途排除、工程驻点完工前后及保存重开。Macro March 35 项、Field R2 72 项、两条正式路线、跨进程持久化以及两个图形 GUI 入口检查均通过。图形测试使用独立临时 `user://` 与 V5 存档；没有操作或关闭旧候选窗口，也没有启动新的长期试玩候选。基础派兵结项、停滞调查 **NOT REPRODUCED / NOT DIAGNOSED** 与玩家验收 **OPEN** 保持不变。详见 `docs/milestones/txwzs-field-tactics-r2/evidence/20260911-location-capabilities/VERIFICATION.md`。
