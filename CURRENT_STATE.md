...
制式长枪、绵甲和轻行装具通过现有木材事务确定性制造并装配，分别进入正式攻防与道路耗时；已经发布的军令保存该次耗时，不受城内后续换装影响。将领装备保留六个唯一槽位，R0 提供佩剑、札甲和战靴三件实际内容并进入同一正式战斗或行军参数。贸易提供两项有库存、容量和当日限额预览的一次成交，资源和唯一回执在同一保存边界提交，不能通过重复点击或读档重复获利。

V5 战役快照升级到 schema 15。旧档获得严格的空战略记录，不补发文官、能量、装备或贸易收益；新档保存任命、剩余能量、限时效果、装备唯一归属、六槽装配和交易回执。定向实现检查、三进程冷恢复、三分辨率图形入口、正式路线 A/B、外部入侵—治疗—反攻链、宏观围城胜利及 Field R2 回归已通过。`run_macro_march_r0_smoke.gd` 的三个地点详情／驻点续令断言在父提交 `82fbe32` 的干净副本中同样失败，因此记录为既有基线问题，不冒充本轮绿色结果。证据见 `docs/milestones/field-tactics-r2/evidence/20260912-city-strategy-r0/VERIFICATION.md`。真人连续体验、装备成长／升阶内容和后三个将领槽位的实际物品仍未完成；玩家验收保持 **OPEN**，此前停滞问题仍为 **NOT REPRODUCED / NOT DIAGNOSED**。
