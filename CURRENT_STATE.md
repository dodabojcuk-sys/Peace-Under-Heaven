...
常态内城、R2 外城战区与 C0 战斗场景继续保持独立：常态城市仍拥有永久建筑、资源、训练与科技；外城仍拥有道路、工程、军令与遭遇；C0 只拥有一次战斗的临时模拟。正式出征进入 C0 后，玩家可在开战前选择瞭望台（木材 6）和攻城槌（木材 8）。确认后由 `NationState` 通过 `ConstructionController` 一次扣除木材，计划只写入同一份 `expedition_attempt`；它不会生成永久城建或外城工程。瞭望台提供战场敌情观察事实，攻城槌在战斗会话初始化时对指定城门造成真实 160 点伤害。
