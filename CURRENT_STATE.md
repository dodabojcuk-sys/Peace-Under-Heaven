...
季节沿用城市日历：秋季显示入冬预警，冬季降低 8 点有效住房容量。相关数值集中在 `blackstone_city_governance_r0.tres`，仍是可回滚的开发参数。定向实现检查、三进程恢复、三分辨率图形检查、正式战争路线 A/B 及入侵恢复链通过。图形证据使用引擎 GUI 按钮信号与受控加速日结算，不是真人正常速度验收；玩家验收保持 **OPEN**。文官能力、装备和贸易仍是后续功能缺口。
