...
V5 当前为 schema 10：活动战时实例保存战斗刻、小队血量/位置、路线、已接受和待执行命令、撤退与任务事实，不保存 Node、动画、相机或 UI 选择。非终局战斗刻经既有运行时发布入口形成检查点；新场景或 V5 恢复后会从同一刻继续。V7/V8 旧档迁移为空工事/空会话快照，不赠送资源；没有旧会话事实的历史 ACTIVE 出征维持原有从 tick 0 重建行为。篡改位置、路线或身份的活动会话被严格拒绝，重复小队、错误数值类型、以及和已接受军令同 ID 但内容冲突的待执行命令同样会被拒绝，不能污染城市状态。宏观围城的 RESERVED 阶段现在也在同一 C0 工事面板中确认计划：费用经 NationState 的独立建设事务一次扣除，计划替换该围城唯一冻结请求后才允许激活；它不会建立常态城市出征或再次扣行军粮。独立 A→B 进程链现在从可见宏观工事面板确认瞭望台，A 保存施工第 1/2 刻，B 冷启动读取相同施工态并在下一正式刻完成；没有重新建计划或第二次扣木材。守城定向 smoke 进一步覆盖施工完成后的拒马受损、正式维修、ACTIVE 冷恢复和维修完成，属于同一进程场景切换与 V5 恢复证据；独立进程防守链仍待补齐，不能与既有 V5 通用冷恢复测试混称。
