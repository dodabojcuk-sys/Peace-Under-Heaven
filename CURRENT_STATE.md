...
这一 R0 是**攻城战时实例基础**，不是“完整防守内城”宣称：临时防御建筑、城内布防、修墙、敌方主动攻城和字段/遭遇入口仍待在同一事实所有权下继续实现。受损或摧毁的临时工事现在在活动 C0 侧栏有正式“维修”入口：资源先由 `ConstructionController` 的既有事务扣除，随后同一 `BattleSession` 记录才进入 `REPAIRING` 并立即保存；保存失败会同时回滚资源和会话状态，重复点击不会重复扣费。入口同样认可任何有效的并行宏观围城接管事务。独立 A→B→C→D→E 进程链已覆盖施工中保存、完工、真实受损、维修中冷恢复、维修完成、待回写和一次回写；它仍不代替独立防守流程，后者继续未完成。`tests/run_wartime_inner_city_r0_smoke.gd`、C0、正式出征因果和 V5 持久化回归已通过；它们是引擎/逻辑验证，不替代玩家体验验收。玩家验收保持 **OPEN**。
