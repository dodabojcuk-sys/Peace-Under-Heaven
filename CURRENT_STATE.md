...
这一 R0 是**攻城战时实例基础**，不是“完整防守内城”宣称：临时防御建筑、城内布防、修墙、敌方主动攻城和字段/遭遇入口仍待在同一事实所有权下继续实现。`tests/run_wartime_inner_city_r0_smoke.gd`、C0、正式出征因果和 V5 持久化回归已通过；它们是引擎/逻辑验证，不替代玩家体验验收。玩家验收保持 **OPEN**。
