...
- `P1_E_FIRST_WAR_CLOSED_LOOP_READY_FOR_USER_GATE`
...
- P1-E 已把第 6 日预警、第 7 日不可绕过的战争阻断、军令台正式入口、同一 C0 北坡战场和三种战果写回串成第一个完整战争闭环。
- 未用假按钮、固定成功或把平衡代理升级为第二套运行时胜负真相；P1-F 完整平衡模拟和用户实体战争体验门禁尚未执行。
...
- P1-E 已复用 C0 形成正式首战入口；P1-F 继续暂停，C0 的路线意义、节奏和命令手感仍等待用户体验门禁。
...
- 未实现外城、多战役、随机事件、通用失败重开或第二场特殊战争；P1-F、首图平衡模拟和用户实体体验门禁仍未完成。
...
- 当前完整回归共 17 个 Godot smoke runner、908 条显式断言全部通过；新增覆盖 180 秒日长、三档倍速、第 6／7 日门禁、正式 C0 串联、四种首战结算路径和战果幂等。
