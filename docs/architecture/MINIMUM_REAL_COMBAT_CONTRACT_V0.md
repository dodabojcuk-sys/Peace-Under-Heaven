...
用途：为 `P1-E` 定义一个真实、可交互、可重复验证的最小战斗事务边界。用户已接受本契约；C0 独立灰盒已实现，正式军令台串联仍属于暂停中的 P1-E。
...
- 契约制定时不存在战斗场景、战斗会话、结果回调、奖励执行器或跨场景事务；C0 后续按本文新增了独立灰盒和同进程事务实现。
...
| 首通奖励的具体数值 | 修改后沿用 | 用户已接受木材 30、粮食 20 作为 C0/P1 首轮可调基线，仅首次胜利应用一次。 |
...
- 城市根保留、战斗场景临时挂载；
- 使用下述双路线、固定 tick 规则；
- 首通奖励采用用户已确认的木材 30、粮食 20 首轮基线，只在首次胜利确认时应用一次。
