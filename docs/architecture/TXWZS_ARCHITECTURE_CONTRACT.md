...
人口 schema 3 将年龄、性别、健康和岗位视为不同维度。儿童、老年、
可用成年人、各岗位、训练预留、居民病患、待安置难民、伤员、军队与
专员必须在互斥守恒式中各计一次；性别仅为与总人口相等的正交聚合，
不得自行改变征募或恢复规则。住房只提供容量，出生和年龄进度只能由
城市日边界推进。难民必须有有限来源和稳定事件 ID，接纳与安置分开；
安置后的伤病进入既有居民疾病和共享医疗容量，不建立第二套治疗账本。

`CityGovernanceState` schema 2 是社会压力、稳定事件身份及难民决定的
唯一持久状态。盗窃、生产干扰和局部停工只作用于各自记录的真实对象，
每阶段后果只应用一次；内乱不得转移主城控制权或生成无来源战斗。城市
治理只解决明确事件并部分缓解压力，不能替代粮食、住房和医疗经营。
V5 campaign schema 16 对旧 schema 15 做保守迁移：保留总数、岗位、
驻军、外派、专员、伤员和已知病患，不伪造历史年龄或性别。
...
| 城市暂停 | 不推进 | 不适用 |
| 战争阻断等待战斗 | 不推进 | 不适用 |
| C0/遭遇战实例 | 不由城市 `_process` 推进；城市、行军与战区共同冻结 | 城防/旧远征只补城市时间；宏观围城不补时；两类都不补战区时间 |
