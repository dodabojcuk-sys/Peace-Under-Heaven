...
| Wartime inner city | C0 has a distinct battle scene, two routes, gates, deployment, deterministic battle and atomic settlement | `enter_first_war_battle()` / formal C0 scene | Battle-only facility planning, observation/fire/repair, active-instance restore and field-siege entry are incomplete | Prebattle plan -> battle effects -> settlement -> independent restore |
