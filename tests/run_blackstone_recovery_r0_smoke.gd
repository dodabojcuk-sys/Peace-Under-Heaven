...
func _run() -> void:
	...
	_check(int(initial.total_living) == 72 and int(initial.available) == 28 and int(initial.production_workers) == 12 and int(initial.construction_workers) == 12 and int(initial.military) == 20 and bool(initial.accounted), "正常开局只有一份人口真值，驻军、岗位与可用人口守恒")
	...
	_check(bool(moved.get("success", false)) and int(after_move.production_workers) == 10 and int(after_move.available) == 30 and city.get_workforce_modifier_permille(&"production") == 833 and city.get_workforce_modifier_permille(&"construction") == 1000, "调岗立即改变正式生产效率，人员不会同时留在可用池")
	...
	_check(bool(training.get("success", false)) and int(queued.training_reserved) == 5 and int(queued.available) == 25 and int(city.food) == food_before_training - 5 * city.INFANTRY_ROLE.recruit_food_per_unit, "训练从可用人口预留人员并支付现有兵种粮食成本")
