...
func _run() -> void:
	...
	_check(int(j.exit_code) == 0 and int(k.exit_code) == 0 and int(l.exit_code) == 0, "三个独立进程完成转移中、驻点等待和返回中的断路军令恢复")
	_check(int(j.exit_code) == 0 and int(k.exit_code) == 0 and int(l.exit_code) == 0, "跨进程实盘保留临时路径、原军令、驻点等待和维修后的返回进度")
	_remove_tree(absolute_directory)
