...
func _check_v5_roundtrip_and_v3_migration() -> void:
	...
	_check(restored.success and target.export_v5_campaign_snapshot() == snapshot, "场景8：当前 schema 精确恢复 legacy 施工、扣料、优先级、治安与主线压力")
	var legacy := _to_v3(snapshot)
	var migrated: Dictionary = target.validate_v5_campaign_snapshot(legacy)
	_check(
		migrated.valid
			and int(migrated.snapshot.schema_version) == V5CampaignSnapshot.SCHEMA_VERSION
