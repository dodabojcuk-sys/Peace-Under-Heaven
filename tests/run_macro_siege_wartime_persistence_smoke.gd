...
const WORKER_PATH := "res://tests/macro_siege_wartime_persistence_worker.gd"
...
func _run() -> void:
	var save_directory := "%s/txwzs-macro-siege-wartime-%d-%d" % [OS.get_temp_dir(), OS.get_process_id(), Time.get_ticks_usec()]
...
func _require(condition: bool, description: String) -> void:
	...
	push_error("MACRO_SIEGE_WARTIME_PERSISTENCE_FAIL: %s" % description)


func _finish() -> void:
	if failures.is_empty():
		print("MACRO_SIEGE_WARTIME_PERSISTENCE PASS assertions=9")
		quit(0)
		return
	quit(1)
