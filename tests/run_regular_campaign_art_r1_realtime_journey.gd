...
func _run() -> void:
	...
	_expect(speed_button != null, "normal 4x speed control is available")
	await _click_control(speed_button)
	await create_timer(1.5).timeout
	var pause_button := _find_button(view, "暂停")
	_expect(pause_button != null, "normal pause control is available")
	await _click_control(pause_button)
	var paused_progress := int(runtime.data.project.progress_ms)
	await create_timer(2.0).timeout
	view.refresh(true)
	_expect(int(runtime.data.project.progress_ms) == paused_progress, "paused construction does not advance")
	var paused_visual := view._map._presentation._building_visuals.get(runtime.data.project.id) as GrayboxBuildingVisual
	_expect(is_instance_valid(paused_visual) and not paused_visual.construction_effect_active, "paused construction effect stops")
	await _capture("02-real-farm-construction-paused-1280x720.png")
	await create_timer(1.4).timeout
	speed_button = _find_button(view, "4×")
	await _click_control(speed_button)
	var construction_deadline := Time.get_ticks_msec() + 35000
	...
	var production_deadline := Time.get_ticks_msec() + 55000
	while int(runtime._stock().food) <= 30 and Time.get_ticks_msec() < production_deadline:
		await create_timer(0.5).timeout
		view.refresh(true)
	await process_frame
	...
	var return_button := _find_button(view, "返回本关战区")
	_expect(return_button != null, "normal return-to-theater control is available")
	await _click_control(return_button)
	await create_timer(1.4).timeout
	base_point = Dictionary(Dictionary(view._model.get("points", {})).get(&"blackstone_city", {}))
	await _click_map_local(view._map._screen(Vector2(base_point.get("world_position", Vector2.ZERO))))
	await create_timer(0.5).timeout
