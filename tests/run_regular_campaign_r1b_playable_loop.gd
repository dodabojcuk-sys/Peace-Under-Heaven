...
func _run() -> void:
	...
	_expect(_find_label_prefix(view, "开局说明") != null, "R1B.2-2: opening identity note is shown to first-time player")
...
func _watchdog() -> void:
	await create_timer(300.0).timeout
	print("R1B_TIMEOUT run exceeded 300 real seconds; forcing quit")
	quit(2)
...
func _scroll_sidebar_to(control: Control) -> void:
	# 确认按钮位于侧栏折叠线以下时，先把侧栏滚到可见区域再点击。
	var rect := control.get_global_rect()
	if rect.position.y < 0.0 or rect.end.y > 700.0:
		view._sidebar.scroll_vertical += int(rect.end.y - 680.0)
		await _hold_frames(3)
