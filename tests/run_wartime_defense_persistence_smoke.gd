...
func _run() -> void:
	var save_directory := "%s/txwzs-wartime-defense-%d-%d" % [OS.get_temp_dir(), OS.get_process_id(), Time.get_ticks_usec()]
	_require(DirAccess.make_dir_recursive_absolute(save_directory) == OK, "建立隔离守城存档目录")
	...
	if failures.is_empty():
		print("WARTIME_DEFENSE_PERSISTENCE_SMOKE PASS assertions=6")
		quit(0)
		return
	quit(1)
